...
void Game::drawChessBoard()
{
    ...
    drawDeadHolder(0,0,Qt::gray);
    drawDeadHolder(1100,0,Qt::gray);
...
void Game::displayMainMenu()
{
    ...
    int gxPos = width()/2 - playButton->boundingRect().width()/2;
    int gyPos = 450;
    InfoButton->setPos(gxPos,gyPos);
    connect(InfoButton, SIGNAL(clicked()),this,SLOT(GameInfo()));
    addToScene(InfoButton);
    listG.append(InfoButton);
...
void Game::GameInfo()
{
        for(size_t i =0, n = listG.size(); i < n; i++)
        removeFromScene(listG[i]);
        QMessageBox::information(this,"Game Info","This is a Project concept for our final Exam it is not made from scratch by us but we did alot of changes in it");
        displayMainMenu();
